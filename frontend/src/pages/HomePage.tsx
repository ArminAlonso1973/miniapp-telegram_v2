...
interface HomePageProps {
  onNavigate: (page: 'assistant' | 'legal-library' | 'report-generator') => void; // Función para cambiar de página
}
...
const HomePage: React.FC<HomePageProps> = ({ onNavigate }) => (
  <div className="min-h-screen bg-gray-900 p-6">
    ...
    <main className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
      {[
        { title: 'Asistente Tributario', page: 'assistant' }, // Opción 1
        { title: 'Biblioteca Legal', page: 'legal-library' }, // Opción 2
        { title: 'Generador de Informes', page: 'report-generator' }, // Opción 3
      ].map((item, index) => (
        <section
          key={index}
          // Cambia de página al hacer clic en una opción
          onClick={() => onNavigate(item.page as 'assistant' | 'legal-library' | 'report-generator')}
